...
func (m *EventModel) Insert(event *Event) (*Event, error) {
	...
	const query = `INSERT INTO events (owner_id, name, description, date, location) VALUES (?, ?, ?, ?, ?)`
...
func (m *EventModel) Update(event *Event) error {
	...
	const query = `UPDATE events SET owner_id = ?, name = ?, description = ?, date = ?, location = ? WHERE id = ?`
